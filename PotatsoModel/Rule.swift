...
public final class Rule {
    ...
    public convenience init() {
        self.init(type: RuleType(rawValue: "NONE")! , action: RuleAction(rawValue: "NONE")!, value: "")
    }
    public convenience init(str: String) throws {
        let data: [String: Any]
        do {
            data = try Rule.getValueFromString(str)
        }
        guard let type = data["type"] as? RuleType, let action = data["action"] as? RuleAction, let value = data["value"] as? String else {
    ...
    public func setFromString(str: String) throws -> Self {
        let data: [String: Any]
        do {
            data = try Rule.getValueFromString(str)
        }
        ...
        setValue(type: type, action: action, value: value)
        return self
    }
    
    fileprivate func setValue(type: RuleType, action: RuleAction, value: String) {
        self.type = type
        self.value = value
        self.action = action
    }
    
    fileprivate static func getValueFromString(_ str: String) throws -> [String: Any]{
        ...
        return ["type": type, "action": action, "value": value]
    }
