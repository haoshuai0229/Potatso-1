...
open class Manager {
    ...
    open func switchVPNFromTodayWidget(_ context: NSExtensionContext) {
        if let url = URL(string:  Potatso.mainScheme() + "://switch") {
    ...
    func setupConfFiles() {
        ...
        func generateConfFile(fileName: String, setName: String)  {
            
            var setArray = DBUtils.allNotDeleted(RuleSet.self, filter: "name == \"\(setName)\"");
            guard setArray.count == 0 else {
                print("\(setName) did exist")
                return

            }
            
            func typeCase(_ string: String.SubSequence, type: String ) -> Bool {
            ...
            DispatchQueue.global().async {
                //全局并发同步
                do {
                    guard let fromURL = Bundle.main.url(forResource: fileName, withExtension: "conf") else {
                        return
                    }
                    let contents = try! String(contentsOf: fromURL)
...
//                        let lineString = lines[index]
                        ...
                        autoreleasepool {
                            do {
                                switch lineString {
                                case let line where typeCase(line, type: "||"):
                                    rule = try Rule(str: String("DOMAIN-SUFFIX, \(line.replacingOccurrences(of: "||", with: "")), PROXY"))
                                case let line where typeCase(line, type: "@@||"):
                                    rule = try Rule(str: String("DOMAIN-SUFFIX, \(line.replacingOccurrences(of: "@@||", with: "")), DIRECT"))
                                case let line where typeCase(line, type: "."):
                                    let startSlicingIndex = line.index(line.startIndex, offsetBy: 1)
                                    let subvalues = line[startSlicingIndex...]
                                    rule = try Rule(str: String("DOMAIN-SUFFIX, \(subvalues), PROXY"))
                                case let line where typeCase(line,type: RuleType.Domain.rawValue),
                                     let line where typeCase(line,type: RuleType.DomainMatch.rawValue),
                                     let line where typeCase(line,type: RuleType.DomainSuffix.rawValue),
                                     let line where typeCase(line,type: RuleType.GeoIP.rawValue),
                                     let line where typeCase(line,type: RuleType.IPCIDR.rawValue),
                                     let line where typeCase(line,type: RuleType.URL.rawValue),
                                     let line where typeCase(line,type: RuleType.URLMatch.rawValue):
                                    rule = try Rule(str: String(line))
                                case let line where typeCase(line, type: "!"),
                                     let line where typeCase(line, type: "#"),
                                     let line where typeCase(line, type: "["): //注释
                                    break
                                default:
                                    rule = try Rule(str: String("DomainMatch, \(lineString), PROXY"))
                                }
                            } catch {
        ...
        //自动代理模式
        generateConfFile(fileName: "Rule", setName: "Auto");
        //黑名单模式
        generateConfFile(fileName: "RuleBlack", setName: "RuleBlack");
        //白名单模式
        generateConfFile(fileName: "RuleWhite", setName: "RuleWhite");
        //回国模式
        generateConfFile(fileName: "RuleBlack_abord", setName: "RuleBlack_abord");
...
extension Manager {
    ...
    func generateSocksConfig() throws {
        ...
        let port = XMLElement.element(withName: "port", children: nil, attributes: [XMLNode.attribute(withName: "value", stringValue: "0") as! XMLNode])  as! XMLElement
        root.addChild(port)
        
        let maxbindwait = XMLElement.element(withName: "maxbindwait", children: nil, attributes: [XMLNode.attribute(withName: "value", stringValue: "10") as! XMLNode]) as! XMLElement
        root.addChild(maxbindwait)
        
        
        let authchoice = XMLElement.element(withName: "authchoice") as! XMLElement
        let select = XMLElement.element(withName: "select", children: nil, attributes: [XMLNode.attribute(withName: "mechanism", stringValue: "anonymous") as! XMLNode])  as! XMLElement
        ...
        if let upstreamProxy = upstreamProxy {
            let chain = XMLElement.element(withName: "chain", children: nil, attributes: [XMLNode.attribute(withName: "name", stringValue: upstreamProxy.name) as! XMLNode]) as! XMLElement
            switch upstreamProxy.type {
            case .Shadowsocks:
                let uriString = "socks5://127.0.0.1:${ssport}"
                let uri = XMLElement.element(withName: "uri", children: nil, attributes: [XMLNode.attribute(withName: "value", stringValue: uriString) as! XMLNode]) as! XMLElement
                chain.addChild(uri)
                let authscheme = XMLElement.element(withName: "authscheme", children: nil, attributes: [XMLNode.attribute(withName: "value", stringValue: "anonymous") as! XMLNode]) as! XMLElement
    ...
    fileprivate func loadAndCreateProviderManager(_ complete: @escaping (NETunnelProviderManager?, Error?) -> Void ) {
        NETunnelProviderManager.loadAllFromPreferences { [unowned self] (managers, error) -> Void in
            if let managers = managers {
                ...
                manager.localizedDescription = Bundle.main.object(forInfoDictionaryKey: "CFBundleDisplayName") as? String
                manager.protocolConfiguration?.serverAddress = Bundle.main.object(forInfoDictionaryKey: "CFBundleDisplayName") as? String
